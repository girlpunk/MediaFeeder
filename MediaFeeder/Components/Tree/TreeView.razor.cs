...
public sealed partial class TreeView
{
    ...
    [Inject]
    public required ILogger<TreeView> Logger { get; set; }
    ...
    private async Task UpdateTree()
    {
        ...
        UnwatchedCache = await context
            .Videos.TagWith("TreeView Unwatched & Downloaded")
            .Where(v => v.Subscription!.UserId == user.Id)
            .GroupBy(
                static v => v.SubscriptionId,
                static (k, g) =>
                    new
                    {
                        key = k,
        ...
        Folders = await context
            .Folders.TagWith("TreeView Folders")
    ...
    private void EditFolder(int? folder)
    {
        var modalConfig = new ModalOptions
        {
            ...
            Title = folder != null ? $"Edit Folder" : "Create Folder",
            DestroyOnClose = true,
        };
    ...
    private void EditSubscription(int? subscription)
    {
        var modalConfig = new ModalOptions
        {
            Title = subscription != null ? $"Edit Subscription" : "Create Subscription",
            AfterClose = async () => await UpdateTree(),
            DestroyOnClose = true,
        };
    ...
    private void EditSelected()
    {
        ...
        {
            Logger.LogWarning(
                "Edit clicked when both SelectedFolder and SelectedSubscription are null"
            );
        }
    }
    ...
    private async Task DeleteSelected()
    {
        ...
        if (SelectedFolder != null)
        {
            ...
            {
                await MessageService.WarnAsync(
                    $"Can not delete a folder that contains subfolders({folder.Subfolders.Count}) or subscriptions({folder.Subscriptions.Count})."
                );
                Logger.LogWarning(
                    "Folder {Folder} contains: {Subfolder} {Subscription}",
                    folder.Id,
                    folder.Subfolders.FirstOrDefault()?.Id,
                    folder.Subscriptions.FirstOrDefault()?.Id
                );
                return;
            }
