...
) : IConsumer<SynchroniseSubscriptionContract<CCCProvider>>
{
    ...
    {
        var url = new UriBuilder(new Uri(subscription.ChannelId)) { Query = $"page={pageNumber}" };
    ...
    private async Task ProcessConferencePage(
    ...
    )
    {
        ...
        foreach (
            var conferenceEvent in conference.Events.Where(conferenceEvent =>
                conferenceEvent.UpdatedAt >= subscription.LastSynchronised
            )
        )
            await SyncVideo(conferenceEvent, subscription, client, cancellationToken);
    }

    private async Task<bool> ProcessEventsPage(
    ...
    )
    {
        subscription.Thumbnail =
            "https://media.ccc.de/assets/frontend/voctocat-header-b587ba587ba768c4a96ed33ee72747b9a5432b954892e25ed9f850a99c7d161c.svg";
    ...
    )
    {
        ...
        video.DownloadedPath =
            recordings
                ...
                .MaxBy(static r => r.Width * r.Height)
